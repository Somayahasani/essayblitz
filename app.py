...
with st.container():
    ...
    # Guidance messages
    essay_message = "Paste your essay here… (minimum 80 words)"
    prompt_message = "e.g.\nPrompt 5: Discuss an accomplishment…\nor\nWhy Stanford?"

    # Essay text area with default value
    with col1:
        essay = st.text_area(
            "Your full essay",
            value=essay_message,  # use value instead of placeholder
            height=380,
            key="essay_input"
        )
    ...
    with col2:
        custom_prompt = st.text_area(
            ...
            value=prompt_message,  # use value instead of placeholder
            height=200,
            key="prompt_input"
        )
